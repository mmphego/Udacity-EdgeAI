...
def draw_masks(result, width, height):
    ...
    classes = cv2.resize(
        result[0].transpose((1, 2, 0)), (width, height), interpolation=cv2.INTER_NEAREST
    )
...
def infer_on_video(args, model):
    ...
    while cap.isOpened():
        ...
        if plugin.wait() == 0:
            ...
            mqttc.publish(
                topic="class", payload=json.dumps({"class_names": class_names})
            )
            mqttc.publish(topic="speedometer", payload=json.dumps({"speed": speed}))
